  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/checkout", {
        ...
        body: JSON.stringify({
          planId: selectedPlan, // Uses standard plans
          sessionId: "", // Special offer might not have a session ID
          ...
          <div className="p-8 pt-0 space-y-10">
            ...
            <div className="bg-purple-50/50 rounded-2xl p-6 border border-purple-100 space-y-4">
              ...
              <button 
                ...
                disabled={isLoading}
                className="w-full bg-purple-600 text-white py-5 rounded-2xl font-black text-xl shadow-lg shadow-purple-200 hover:bg-purple-700 transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
              >
                {isLoading ? (
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center">
        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 ${isActive ? 'border-purple-600' : 'border-slate-200'}`}>
          {isActive && <div className="w-3 h-3 bg-purple-600 rounded-full" />}
        </div>
        <div className="ml-3 flex-1">
          <h3 className={`font-bold text-base transition-colors ${isActive ? 'text-purple-900' : 'text-slate-900'}`}>{title}</h3>
        ...
        <div className="text-right ml-3">
          ...
          <p className={`text-xl font-black ${isActive ? 'text-purple-700' : 'text-slate-900'}`}>${price}</p>
        </div>
      </div>
    </div>
  );
